...
const App = () => {
  ...
  // Clean value get to from keyboard (language: Vietnamese) 🔥
  // const xoaDau = str => {
  ...
  const url = `${API.base}weather?q=${(location.toLowerCase())}&appid=${API.key}&lang=vi&units=metric`;
  ...
  const checkData = (location) => {
    let count = 0;
    CityName.map(data => {
      if (data.city.toLowerCase() === location || data.city.toLowerCase() === location.toLowerCase() || (data.city.toLowerCase()) === location) {
        count++;
      }
    })
    if (count === 0) {
